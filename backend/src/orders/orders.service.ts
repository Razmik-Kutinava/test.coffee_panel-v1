...
export class OrdersService {
  ...
  async findAll(status?: OrderStatus, locationId?: string) {
    ...
    // Загружаем заказы без связанного пользователя, чтобы избежать ошибки enum 'customer'
    // Prisma Client валидирует enum на клиентской стороне при include user
    ...
    // Загружаем пользователей отдельно через raw SQL, нормализуя роль 'customer' -> 'client'
    ...
    const users = await client.$queryRawUnsafe(`
      SELECT 
        ...
        phone,
        email,
        CASE WHEN role::text = 'customer' THEN 'client' ELSE role::text END as role,
